...
export class EntryEntity implements PrismaEntry {
    ...
    @ApiProperty({
        description: 'The main content or body of the entry, typically a message or file reference',
        example: 'This is a sample text entry',
    })
    content: string;

    @ApiProperty({
        description: 'Metadata related to the entry, such as additional info or settings. Can be any object',
        required: false,
        type: Object,
    ...
    @ApiProperty({
        description: 'Timestamp when the entry was read by the user (if applicable)',
        required: false,
