...
export class EntryResponseDto {
    ...
    @ApiProperty({
        description: 'The main content or body of the entry, which could be a message, notification, or file reference',
        example: 'This is a sample content for the entry.',
    })
    content: string;

    @ApiProperty({
        description: 'Optional metadata related to the entry, such as additional context like source or priority. Can be any object',
        required: false,
        type: Object,
    ...
    @ApiProperty({
        description: 'Timestamp when the entry was logged or recorded in the system',
        example: '2023-04-15T10:00:00Z',
    })
    loggedAt: Date;

    @ApiProperty({
        description: 'Timestamp when the entry was read (if applicable). Can be null if not yet read.',
        required: false,
