...
import { IPaymentProvider, PaymentInitiationResponse, PaymentVerificationResponse } from '../interfaces/payment-provider.interface';
...
export class FlutterwaveProvider implements IPaymentProvider {
    ...
    constructor(private configService: ConfigService) {
        ...
        this.baseUrl = this.configService.get('FLUTTERWAVE_BASE_URL', 'https://api.flutterwave.com/v3');
    }

    getProviderName() { return PaymentProvider.FLUTTERWAVE };
    ...
    async initiatePayment(
        ...
        try {
            ...
            console.error('Flutterwave payment initiation error:', error.response?.data);
    ...
    async verifyPayment(reference: string): Promise<PaymentVerificationResponse> {
        try {
            ...
            console.error('Flutterwave payment verification error:', error.response?.data);
    ...
    async handleWebhook(payload: any, signature: string): Promise<boolean> {
        ...
        return payload.event === 'charge.completed' &&
