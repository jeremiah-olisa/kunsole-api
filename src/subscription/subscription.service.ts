...
export class SubscriptionService {
  ...
  async createSubscription(dto: CreateSubscriptionDto, userId: string) {
    ...
    const userApp = await this.userAppService.findByUserAndApp(
      userId,
      dto.appId,
    );
    const permiddedRoles: AppUserRole[] = [
      AppUserRole.OWNER,
      AppUserRole.ADMIN,
    ];
    ...
    if (!userApp || !permiddedRoles.includes(userApp.role)) {
      throw new NotFoundException(
        'You do not have permission to create subscriptions for this app',
      );
    }
  ...
  private async activateNextQueuedSubscription(
    ...
    const currentSub = await this.repository.findById(
    ...
    if (nextQueued) {
      await this.repository.update(nextQueued.id, { isQueued: false }, tx);
    }
  }
